...
public class EnterHtmlFormFragmentRequestMapper implements FragmentRequestMapper {
  ...
  /**
   * Method used to determine if custom handling should be used. When encounter datetime validation
   * is disabled, the default fragment provider (handler) is overridden by the custom one.
   *
  ...
  public boolean mapRequest(FragmentRequest request) {
    if (isHtmlFormFragment(request) && isEncounterDatetimeValidationDisabled()) {
      LOGGER.info(
          String.format(
              "The encounter datetime validation is disabled (see %s global setting). "
                  + "The `%s` fragment from `%s` provider will be change to `%s` fragment from `%s` provider.",
              GlobalPropertiesConstants.ENCOUNTER_DATETIME_VALIDATION.getKey(),
              OLD_FRAGMENT,
              OLD_PROVIDER,
              NEW_FRAGMENT,
              NEW_PROVIDER));
  ...
  private boolean isHtmlFormFragment(FragmentRequest request) {
    return OLD_PROVIDER.equals(request.getProviderName())
        && OLD_FRAGMENT.equals(request.getFragmentId());
  }
