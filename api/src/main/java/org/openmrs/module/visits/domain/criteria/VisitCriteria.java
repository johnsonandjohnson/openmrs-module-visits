...
public class VisitCriteria extends BaseCriteria implements Serializable {
  ...
  public void setOnlyMissedVisitsWithIncorrectStatusRestrictions(
      int minDelaysInDays, String statusOfMissedVisit, List<String> statusesEndingVisit) {
  ...
  public static VisitCriteria forMissedVisits(
      int minDelayInDays, String statusOfMissedVisit, List<String> statusesEndingVisit) {
  ...
  private void addOnlyMissedVisitWithIncorrectStatusRestrictions(Criteria criteria) {
    Date maxStartDate =
        DateUtil.getDateSecondsAgo((long) DateUtil.DAY_IN_SECONDS * minDelaysInDays);

    DetachedCriteria subCriteria =
        DetachedCriteria.forClass(VisitAttribute.class, "va")
            ...
            .add(
                Restrictions.not(
                    Restrictions.in("va.valueReference", statusesForEndedAndMissedVisit)))
            .setProjection(
                Projections.projectionList().add(Projections.property("va.visit.visitId")));

    criteria
        .add(Subqueries.propertyIn("visitId", subCriteria))
        .add(
