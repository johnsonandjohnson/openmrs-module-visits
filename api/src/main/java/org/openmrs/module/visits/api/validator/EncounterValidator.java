...
/**
 * Validator for {@link Encounter} class. This validator is used to disable the validation of
 * encounter datetime (provided by the OpenMRS core validator). The order of the OpenMRS core
 * validator is equaled to 50 that's why this validator has an order set as 100 to be executed after
 * default one. Introduced in order to extend the default logic without changing the OpenMRS core
 * platform.
 */
@Handler(
...
public class EncounterValidator extends BaseEncounterDatetimeOverrideValidator {
  ...
  void handleDisabledEncounterDatetimeValidation(List<FieldError> fieldErrors) {
    ...
    fieldErrors.removeIf(error -> ENCOUNTER_DATETIME_FIELD.equals(error.getField()));
  }
}
